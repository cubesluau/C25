...
--     "local Rayfield = loadstring(game:HttpGet('https://sirius.menu/rayfield'))()" above this line instead. ===
-- ==============================================================================
...
local function registerResync(handle, applyFn)
    if handle and applyFn then
        table.insert(dropdownResync, function() applyFn(handle.CurrentValue) end)
    end
end
...
local function Notify(title, content, kind, dur)
    pcall(function()
        Rayfield:Notify({
            ...
            Content = content,
            Duration = dur or 2.5,
            Image = kind == "Success" and "check" or kind == "Error" and "x" or "info"
        })
    end)
end
...
local function safeCallback(fn)
    return function(...)
        ...
        if not ok then
            pcall(Notify, "Carbonation", "Error: " .. tostring(err), "Error", 4)
        end
    end
end
...
-- ==============================================================================
local EggsTab     = Window:CreateTab("Eggs", "crown")
local BaseTab     = Window:CreateTab("Base", "bolt")
...
EggsTab:CreateToggle({
    Name = "Rare Egg Hunter (Highest Rarity First)",
    CurrentValue = true,
    Flag = "rare_hunter",
...
EggsTab:CreateDropdown({
    Name = "Filter by Rarity (Multi-Select)",
    ...
    Callback = function(selectedList) 
        ...
        for _, v in ipairs(selectedList) do selectedStealRarities[v] = true end
    end
})

EggsTab:CreateDropdown({
    Name = "Filter by Area (Multi-Select)",
    Options = AREA_NAMES,
    CurrentOption = {},
    MultipleOptions = true,
    Flag = "steal_areas",
    Callback = function(selectedList) 
        selectedStealAreas = {}
        for _, v in ipairs(selectedList) do selectedStealAreas[v] = true end
    end
})

EggsTab:CreateDropdown({
    Name = "Filter by Mutation (Multi-Select)",
    Options = MUTATION_FILTERS,
    CurrentOption = {},
    MultipleOptions = true,
    Flag = "steal_muts",
    Callback = function(selectedList) 
        selectedMutationTypes = {}
        for _, v in ipairs(selectedList) do selectedMutationTypes[v] = true end
    end
})

EggsTab:CreateSlider({
    Name = "Glide / Travel Speed",
    Range = {50, 500},
    Increment = 10,
    Suffix = " studs/s",
    CurrentValue = 200,
...
EggsTab:CreateButton({
    Name = "Steal Best Available Egg Once",
...
EggsTab:CreateSection("Auto Hatch & Plant")
...
EggsTab:CreateSection("Egg Tracker ESP")

EggsTab:CreateToggle({
    Name = "Egg ESP Enabled",
    CurrentValue = false,
    Flag = "esp_eggs_enabled",
    Callback = safeCallback(function(v)
        esp.enabled = v
...
EggsTab:CreateToggle({
    Name = "Show Mutated / Rare Eggs Only",
    CurrentValue = false,
    Flag = "esp_eggs_rare_only",
...
-- TAB 2: BASE & UPGRADES
-- -----------------------------------------------------------------------------
BaseTab:CreateSection("Homestead Upgrades")

BaseTab:CreateToggle({
    Name = "Auto Upgrade Base / Plot",
    CurrentValue = false,
    Flag = "up_base_auto",
...
BaseTab:CreateToggle({
    Name = "Auto Upgrade Treadmill Tier",
    CurrentValue = false,
    Flag = "up_tread_auto",
...
BaseTab:CreateSection("Treadmill & Speed")

BaseTab:CreateToggle({
    Name = "Auto Equip Best Pets",
    CurrentValue = false,
    Flag = "equip_best_pets",
...
BaseTab:CreateSection("Claim Rewards")

BaseTab:CreateToggle({
    Name = "Auto Claim Away Earnings & Codex",
    CurrentValue = false,
    Flag = "claim_auto_rewards",
    Callback = function(v) autoClaimRewards = v end
})

BaseTab:CreateButton({
    Name = "Claim Away Earnings & Codex Now",
    Callback = safeCallback(function()
...
-- -----------------------------------------------------------------------------
CombatTab:CreateSection("Bat & Slap Aura")

CombatTab:CreateToggle({
    Name = "Bat / Slap Aura",
    CurrentValue = false,
    Flag = "bat_aura_enabled",
    Callback = safeCallback(function(v)
        batAuraEnabled = v
...
CombatTab:CreateSection("Defense & Guards")

CombatTab:CreateToggle({
    Name = "Anti-Ragdoll (Quick Standup)",
    CurrentValue = false,
    Flag = "anti_ragdoll",
...
PlayerTab:CreateToggle({
    Name = "Smooth Fly (WASD + Space/Shift)",
    CurrentValue = false,
    Flag = "fly_enabled",
    Callback = safeCallback(function(v)
...
PlayerTab:CreateToggle({
    Name = "Anti-AFK (Bypass 20min Kick)",
    CurrentValue = false,
    Flag = "anti_afk",
    Callback = function(v) SetAntiAFK(v) end
})

PlayerTab:CreateSection("Area Travel")
...
PlayerTab:CreateDropdown({
    Name = "Select Area",
    Options = areaKeys,
    CurrentOption = {"Base / Plot"},
    Flag = "tele_area",
    Callback = function(v) 
        selectedAreaTp = v[1] or v
    end
})

PlayerTab:CreateButton({
    Name = "Travel to Selected Area",
    Callback = safeCallback(function()
...
PlayerTab:CreateSection("Plot Travel")
...
PlayerTab:CreateDropdown({
    Name = "Select Plot",
    Options = plotOptions,
    CurrentOption = {"My Plot"},
    Flag = "tele_plot",
    Callback = function(v) 
        selectedPlotNum = v[1] or v
    end
})
...
PlayerTab:CreateSection("Player Travel")
...
SettingsTab:CreateInput({
    ...
    CurrentValue = "stealanegg",
    Flag = "cfg_name",
    Callback = function(v) 
        if v and #v > 0 then 
            -- Rayfield handles config names automatically via ConfigurationSaving
        end 
    end
})
...
SettingsTab:CreateParagraph({
    Title = "Carbonation | Ein Ei stehlen",
